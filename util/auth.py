...
def check_key(api_key_from_header: str = Security(api_key_header)):
    log.info(f"util/auth.py check_key: X-API-Key {api_key_from_header}")
    if mutil.key_exists(api_key_from_header):
        return True
    raise HTTPException(
