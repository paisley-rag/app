...
def extract_from_response(response):
    # creates context using 'text' from each 'node' in response's 'source_nodes'
    # also deletes any instances of '\n'
    if not response:
...
def change_nans_to_zeros(score):
    # sometimes when context and answer don't match,
    # RAGAs gives a 'NaN' faithfulness answer score
    # - which I'll replace with 0.0 for now
